class ZHXCore {
    constructor() {
        ...
        ZHXCore.initResources();
        ZHXCore.handleCounterUp();
        ZHXCore.initMediaIntegrate();
        if (ZHXCoreVariables && ZHXCoreVariables.authorized === '0') {
    ...
    static showNotice(messageType, message) {
        ...
        switch (messageType) {
            case 'error':
                messageHeader = ZHXCoreVariables.languages.notices_msg.error;
                break;
            case 'success':
                messageHeader = ZHXCoreVariables.languages.notices_msg.success;
                break;
        }
    ...
    static handleError(data) {
        if (typeof (data.errors) !== 'undefined' && !_.isArray(data.errors)) {
            ZHXCore.handleValidationError(data.errors);
        } else {
            if (typeof (data.responseJSON) !== 'undefined') {
                if (typeof (data.responseJSON.errors) !== 'undefined') {
                    if (data.status === 422) {
                        ZHXCore.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof (data.responseJSON.message) !== 'undefined') {
                    ZHXCore.showError(data.responseJSON.message);
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            ZHXCore.showError(item);
                        });
                    });
                }
            } else {
                ZHXCore.showError(data.statusText);
            }
        }
    }
    ...
    countCharacter() {
        $.fn.charCounter = function (max, settings) {
            ...
            settings = $.extend({
                ...
                format: '(%1 ' + ZHXCoreVariables.languages.system.character_remain + ')',
                pulse: true,
                delay: 0
            }, settings);
            let p, timeout;
    ...
    static initResources() {
        ...
        ZHXCore.initDatePicker('.datepicker');
        if (jQuery().mCustomScrollbar) {
            ZHXCore.callScroll($('.list-item-checkbox'));
        }
    ...
    handlePortletTools() {
        ...
        $('body').on('click', '.portlet > .portlet-title .fullscreen', event =>  {
            ...
            if (portlet.hasClass('portlet-fullscreen')) {
                ...
                let height = ZHXCore.getViewPort().height -
    ...
    static initCodeEditor(id) {
        ...
        $('.handle-tool-drag').mousedown(event =>  {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', ZHXCore.onDragTool);
            $(window).on('mouseup', ZHXCore.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', ZHXCore.onDragTool);
        $(window).off('mouseup', ZHXCore.onReleaseTool);
    }
